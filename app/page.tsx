...
export default function Dashboard() {
  ...
  const grouped = categories.map(cat => ({ ...cat, items: filtered.filter(r => r.category_id === cat.id) })).filter(c => c.items.length > 0)
  ...
  return (
    <div className="min-h-screen bg-[#f4f4f2]">
      {announcements.length > 0 && (
        <div className="bg-kw-red text-white text-center py-2.5 px-4 text-sm font-medium">
      ...
      <header className="bg-kw-black py-5 px-6">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {isAdmin && <Link href="/admin" className="btn-kw text-xs px-4 py-2 rounded-lg font-semibold">Admin Panel</Link>}
            <Link href="/auth" className="text-gray-400 hover:text-white text-xs transition-colors">Admin login</Link>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-10">
        <div className="space-y-4">
          <div className="relative">
            ...
            <input type="text" placeholder="Search resources..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-kw-red shadow-sm" />
          </div>
        ...
        {grouped.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            <p>{resources.length===0?'No resources yet.':'No resources match your search.'}</p>
          </div>
        ) : grouped.map(cat => (
          <section key={cat.id}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {cat.items.map(r => r.url ? (
                <a key={r.id} href={r.url} target="_blank" rel="noopener noreferrer" className="resource-card bg-white border border-gray-200 rounded-xl p-4 flex flex-col gap-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {partners.map(p => (
                <div key={p.id} className="bg-white border-2 border-kw-red rounded-xl p-5 flex flex-col gap-2">
                  <div><p className="font-bold text-kw-black">{p.name}</p>{p.category && <p className="text-xs text-kw-red font-semibold uppercase tracking-wider">{p.category}</p>}</div>
